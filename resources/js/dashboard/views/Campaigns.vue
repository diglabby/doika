  ...
  <div>
    <b-card>
      <template slot="header">
        <h3 class="card-title">
          {{ $t('labels.admin.campaigns.header') }}
        </h3>
        <div class="card-options">
          <b-button to="/campaigns/create" variant="success" size="sm">
            <i class="fe fe-plus-circle"></i>
            {{ $t('buttons.admin.campaigns.create') }}
          </b-button>
        </div>
      </template>
      <b-datatable
        ...
        action-route="admin.campaigns.batch_action"
        :actions="actions"
      ...
      >
        ...
        >
          <template v-slot:cell(title)="data">
            <router-link
              :to="`/campaigns/${data.item.id}/edit`"
          ...
          <template v-slot:cell(status)="data">
            <c-switch
              ...
              @change="onActiveToggle(data.item.id)"
            ></c-switch>
          </template>
          ...
          <template v-slot:cell(average)="data">
            <span
              v-text="
                average(
                  data.item.transactions_count,
                  data.item.amount_collected
                )
              "
            ></span>
          </template>
          ...
          <template v-slot:cell(received)="data">
            <span
              v-text="
                `${data.item.amount_collected.toFixed(
                  2
                )}/${data.item.target_amount.toFixed(2)}`
              "
            ></span>
          </template>
