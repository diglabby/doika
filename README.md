...
4. Рыхтуем канфіг файл. Для гэтага ў тэчцы doika ствараем копію файла .env.example і надаем яму імя .env (падказка: выканаць каманду cp .env.example .env)
5. З тэрмінала запусціць каманду `composer install`. Вы павінны запусціць з карнявой дырэкторыі doika
6. З тэрмінала запусціць каманду `npm install`. Вы павінны запусціць з карнявой дырэкторыі doika. Пры узнікненні памылак з node-sass выпаўняем `sudo npm install -g --unsafe-perm node-sass` пасля `sudo chown -R $(whoami) ~/.npm`
7. Выконваем зборку фронтэнда `npm run dev`
...
17. Выкарыстоўваеце дадзеныя для ўвахода Лагін: `demo@example.com` , Пароль: `password`
