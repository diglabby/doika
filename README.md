...
2.1. Дадаем змесціва fork рэпазіторыя ў корань вашага сайта з дапамогай git clone. Пераключаемся на  dev branch, з дапамогай git checkout (напрыклад, git checkout dev)
2.2. Неабходна вызначыць групу і карыстальніка для ўсіх файлаў модуля камандай: sudo "chown -R www-data:www-data ."
3. Імпартуем файл doika.sql у базу дадзеных, пры з'яўленні памылак звяртаем ўвагу на версію MySQL.
...
6. Генерым ключ для laravel: зайсцi у папку doika праз i выканаць php artisan key:generate
7. Усталяваная версія павінна запускацца на Вашым лакальным хасце са шляхам doika/admin/login
8. Выкарыстоўваеце дадзеныя для ўвахода Лагін: demo@example.com , Пароль: demo
